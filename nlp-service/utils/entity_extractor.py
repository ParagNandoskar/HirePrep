...
class EntityExtractor:
    ...
    def extract_education(self, text: str) -> List[Dict[str, Any]]:
        ...
        degree_patterns = [
            r'(bachelor|master|phd|doctorate|associate|diploma)\s+(?:of\s+)?(?:science\s+)?(?:arts\s+)?(?:in\s+)?([\w\s]+)',
            r'(b\.?s\.?|m\.?s\.?|ph\.?d\.?|m\.?a\.?|b\.?a\.?)\s+(?:in\s+)?([\w\s]+)',
        ]
    ...
    def _find_nearby_institution(self, text: str, start: int, end: int) -> str:
        """Find institution name near degree mention"""
        ...
        institution_pattern = r'([A-Z][a-z\s]+(?:University|College|Institute|School))'
        match = re.search(institution_pattern, context)
